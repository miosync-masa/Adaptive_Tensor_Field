...
import numpy as np
import pandas as pd
from numba import njit, jit, prange, typed
...
@njit
def safe_values_numba(v_array):
    """配列を安全にfloat32配列に変換（Numba版）"""
    return v_array.astype(np.float32)

@njit
def vectorize_state_numba(
    severity_level, action_magnitude, threat_context,
    trust_score, security_mode, event_score,
    is_confidential_access, is_cross_dept_access,
    is_external_network, permission_level_diff,
    operation_success_flag
):
    """状態パラメータを直接受け取ってベクトル化（Numba版）"""
    vec = np.zeros(11, dtype=np.float64)
    vec[0] = severity_level
    vec[1] = action_magnitude
    vec[2] = threat_context
    vec[3] = trust_score
    vec[4] = security_mode
    vec[5] = event_score
    vec[6] = is_confidential_access
    vec[7] = is_cross_dept_access
    vec[8] = is_external_network
    vec[9] = permission_level_diff
    vec[10] = operation_success_flag
    return vec
...
def vectorize_state(state_params):
    ...
    return vectorize_state_numba(*values)

@njit
def is_global_ip_numba(ip_bytes):
    """IPアドレスがグローバルかチェック（Numba版）"""
    # プライベートIPプレフィックスのバイト表現
    # 192.168., 10., 172.
    if len(ip_bytes) < 3:
        return False
    ...
    if ip_bytes[0] == 192 and ip_bytes[1] == 168:
        ...
        return False
    elif ip_bytes[0] == 172 and 16 <= ip_bytes[1] <= 31:
        return False
...
def is_global_ip(ip: str) -> bool:
    ...
    try:
        ...
        if len(parts) != 4:
            return False
        return is_global_ip_numba(np.array(parts[:3], dtype=np.uint8))
    except:
        return False
...
@njit
def get_adaptive_weight_jit(diff, key_idx, weights_l1, weights_l2):
    """適応的重み取得（JIT版）"""
    if diff < 0.5:
        return weights_l1[key_idx]
    elif diff < 2.0:
        t = (diff - 0.5) / 1.5
        return (1 - t) * weights_l1[key_idx] + t * weights_l2[key_idx]
    else:
        return weights_l2[key_idx]

@njit
def adaptive_metric_jit(diff):
    """適応的メトリック（JIT版）"""
    if diff < 0.5:
        return diff
    elif diff < 2.0:
...
@njit
def compute_security_divergence_jit(
    prev_values, curr_values,
    weights_l1, weights_l2,
    use_adaptive=True
):
    """セキュリティダイバージェンス計算（JIT版）"""
    divergence = 0.0
    n_features = len(prev_values)

    for i in range(n_features):
        diff = abs(curr_values[i] - prev_values[i])

        if use_adaptive:
            w = get_adaptive_weight_jit(diff, i, weights_l1, weights_l2)
            delta = w * adaptive_metric_jit(diff)
        else:
            # L1メトリック
            delta = weights_l1[i] * diff
...
def compute_security_divergence(
    ...
    # デフォルト重み
    if weights_l1 is None or weights_l2 is None:
        return 0.0  # エラー回避
    ...
    # 配列に変換
    prev_values = np.array([float(prev_state.get(k, 0)) for k in FEATURE_KEYS])
    curr_values = np.array([float(curr_state.get(k, 0)) for k in FEATURE_KEYS])
    w1_array = np.array([weights_l1.get(k, 1.0) for k in FEATURE_KEYS])
    w2_array = np.array([weights_l2.get(k, 1.0) for k in FEATURE_KEYS])

    # JIT関数呼び出し
    use_adaptive = metric in ["weighted_hybrid", "adaptive"]
    return compute_security_divergence_jit(
        prev_values, curr_values,
        w1_array, w2_array,
        use_adaptive
    )
...
@njit
def evaluate_clusters_jit(
    state_vector, cluster_centers,
    weights_l1, weights_l2
):
    """全クラスタに対する評価（JIT版）"""
    n_clusters = len(cluster_centers)
    divergences = np.zeros(n_clusters)

    for i in range(n_clusters):
        divergences[i] = compute_security_divergence_jit(
            cluster_centers[i], state_vector,
            weights_l1, weights_l2, True
        )

    best_cluster = np.argmin(divergences)
    min_divergence = divergences[best_cluster]

    return min_divergence, best_cluster, divergences

# Python互換ラッパー
def evaluate_security_event_by_cluster(
    new_state, model, info_tensor,
    weights_l1=None, weights_l2=None,
    ...
    if isinstance(new_state, dict):
        state_vector = np.array([float(new_state.get(k, 0)) for k in FEATURE_KEYS])
    else:
        state_vector = np.array(new_state, dtype=np.float64)
    ...
    # JIT関数呼び出し
    min_divergence, best_cluster, _ = evaluate_clusters_jit(
        state_vector, model.cluster_centers_,
        w1_array, w2_array
    )
...
@njit(parallel=True)
def batch_evaluate_states(
    state_vectors, cluster_centers,
    weights_l1, weights_l2
):
    """複数の状態ベクトルをバッチ評価（並列処理）"""
    n_states = len(state_vectors)
    n_clusters = len(cluster_centers)

    # 結果配列
    best_clusters = np.zeros(n_states, dtype=np.int32)
    min_divergences = np.zeros(n_states)
    ...
    for i in prange(n_states):
        ...
        for j in range(n_clusters):
            divergences[j] = compute_security_divergence_jit(
                cluster_centers[j], state_vectors[i],
                weights_l1, weights_l2, True
            )

        best_clusters[i] = np.argmin(divergences)
        min_divergences[i] = divergences[best_clusters[i]]

    return min_divergences, best_clusters
